...
if [ "$sub_enable" != "0" ];then
  ...
  # Start the subscription process
	...
	# 6. Case with User and Password present, but no ClientID and no Certificate present
		...
		case="6"  
		mosquitto_sub $D -h $server -p $port -q $sub_qos  -t $sub_topic -u $user -P "$pass" $LORA &
	...
	msubcount=$(ps | grep -c '[m]osquitto_sub')
	if [ "$msubcount" -eq 2 ]; then
		subflag="1"
			...
			for channel in $CID; do
				...
				if [ -n "$CHAN_INFO" ];then
					...
					data=$(echo $meta | cut  -d , -f 3-100)		# For late version with metadata from radio
 					...
 					# JSON 
					str=`echo $data | sed 's/{//'` 
					json="{\"timestamp\":\"$time\",\"rssi\":\"$rssi\",\"payload\":\"$data\"}"
